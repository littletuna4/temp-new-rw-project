...
export const Game: GameRelationResolvers = {
  series: (_obj, { root }) => {
    return db.game.findUnique({ where: { id: root?.id } }).series()
  },
  room: (_obj, { root }) => {
    return db.game.findUnique({ where: { id: root?.id } }).room()
  },
  gameInvitation: (_obj, { root }) => {
    return db.game.findUnique({ where: { id: root?.id } }).gameInvitation()
  },
}
