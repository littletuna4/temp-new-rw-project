...
export const Club: ClubRelationResolvers = {
  memberships: (_obj, { root }) => {
    return db.club.findUnique({ where: { id: root?.id } }).memberships()
  },
  organization: (_obj, { root }) => {
    return db.club.findUnique({ where: { id: root?.id } }).organization()
  },
  wallets: (_obj, { root }) => {
    return db.club.findUnique({ where: { id: root?.id } }).wallets()
  },
}
